...
export class PropertyManager {
    ...
    getExpressId(faceIndex: Number) {
      for (const index in this.mapFaceindexID) {
    ...
    getSpatialStructure() {
      const lines = this.ifcAPI.GetLineIDsWithType(this.modelID, IFCPROJECT);
      const ifcProjectId = lines.get(0);
      const ifcProject = this.ifcAPI.GetLine(this.modelID, ifcProjectId);
