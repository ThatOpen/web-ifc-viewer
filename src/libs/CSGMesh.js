...
class Vector extends THREE.Vector3 {
  constructor(x, y, z) {
    if (arguments.length == 3) super(x, y, z);
    else if (Array.isArray(x)) super(x[0], x[1], x[2]);
    else if (typeof x == 'object') super().copy(x);
...
CSG.toMesh = function (csg, toMatrix) {
  ...
  for (var i = 0; i < ps.length; i++) {
    ...
    for (var j = 0; j < pvlen; j++) vs.push(new THREE.Vector3().copy(pvs[j].pos));
...
CSG.ieval = function (tokens, index = 0) {
  if (typeof tokens === 'string') CSG.currentOp = tokens;
    ...
    if (!CSG.sourceMesh) CSG.currentPrim = CSG.fromMesh((CSG.sourceMesh = tokens));
    else {
