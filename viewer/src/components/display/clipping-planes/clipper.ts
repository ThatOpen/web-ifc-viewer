...
export class IfcClipper extends Component {
  dragging: boolean;
  planes: IfcPlane[];
  ...
  constructor(api: IfcViewerAPI) {
    super();
    this.api = api;
    this.enabled = false;
    this.dragging = false;
    this.planes = [];
    api.ifcCaster.setClipper(this);
  }
  ...
  createPlane = () => {
    ...
    const intersects = this.api.ifcCaster.castRayIfc();
  ...
  private pickPlane = () => {
    ...
    const intersects = this.api.ifcCaster.castRay(planeMeshes);
  ...
  private updateMaterials = () => {
    ...
    this.api.items.ifcModels.forEach((obj: Object3D) => {
