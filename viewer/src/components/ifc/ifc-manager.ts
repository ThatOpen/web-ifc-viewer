...
export class IfcManager extends IfcComponent {
  ...
  async loadAlignments2D(modelID: number) {
    await IfcAlignmentsManager.drawAlignments2D(
      await this.loader.ifcManager.ifcAPI.GetAllAlignments(modelID),
      this.context.getScene()
    );
  }

  async loadAlignments3D(modelID: number) {
    await IfcAlignmentsManager.drawAlignments3D(
      await this.loader.ifcManager.ifcAPI.GetAllAlignments(modelID),
      this.context.getScene()
    );
  }
