...
import { Matrix3, Geometry, Vector3, Face3, Matrix4, Mesh, MeshLambertMaterial, MeshBasicMaterial } from 'three';
...
const newToken = createToken; //Tokens / vocabulary for constructing the parser primitives
...
CSG.fromGeometry = function (geom) {
  if (geom.isBufferGeometry) geom = new Geometry().fromBufferGeometry(geom);
  var fs = geom.faces;
...
CSG.toMesh = function (csg, toMatrix) {
  ...
  for (var i = 0; i < ps.length; i++) {
    ...
    for (var j = 0; j < pvlen; j++) vs.push(new Vector3().copy(pvs[j].pos));
  ...
  var inv = new Matrix4().getInverse(toMatrix);
